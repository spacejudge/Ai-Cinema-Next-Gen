...
export default function ServicesPage() {
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="pt-24 md:pt-32 pb-10 md:pb-16 px-4 md:px-8">
        ...
        <div className="border-t-2 border-foreground">
          {services.map((service) => (
            <Link
              key={service.number}
              href={service.href || "#"}
              className="block group border-b-2 border-foreground py-6 md:py-12 hover:bg-foreground hover:text-background transition-colors cursor-pointer"
            >
              <div className="flex flex-col md:flex-row md:items-start gap-3 md:gap-8">
                <span className="font-mono text-primary text-xs md:text-sm group-hover:text-primary">{service.number}</span>
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h2 className="font-serif text-2xl sm:text-3xl md:text-6xl uppercase tracking-tight">{service.title}</h2>
                    <ArrowRight
                      className="hidden md:block transform group-hover:translate-x-2 transition-transform"
                      size={32}
                    />
                  </div>
                  <p className="font-mono text-xs md:text-sm mt-3 md:mt-6 max-w-2xl opacity-70">{service.description}</p>
                  <div className="flex flex-wrap gap-1.5 md:gap-2 mt-3 md:mt-6">
                    {service.deliverables.map((item) => (
                      <span
                        key={item}
                        className="font-mono text-[10px] md:text-xs px-2 py-0.5 md:px-3 md:py-1 border border-current rounded-full uppercase"
                      >
                        {item}
                      </span>
                    ))}
      ...
      <section className="px-4 md:px-8 pb-12 md:pb-24">
        <div className="bg-primary p-6 md:p-16">
          <h2 className="font-serif text-2xl sm:text-3xl md:text-6xl uppercase tracking-tight text-black">Ready to start?</h2>
          <p className="font-mono text-xs md:text-base text-black/70 mt-3 md:mt-4 max-w-xl">
            Let&apos;s discuss your project and see how we can help bring your vision to life.
          </p>
          <a
            href="/contact"
            className="inline-block mt-5 md:mt-8 px-6 py-3 md:px-8 md:py-4 bg-black text-white font-mono text-sm uppercase hover:bg-white hover:text-black transition-colors border-2 border-black"
          >
            Get in Touch
          </a>
        </div>
